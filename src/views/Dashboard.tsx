...
export default function Dashboard() {
  ...
  const datos = useMemo(() => {
    ...
    const semana = Array.from({ length: 7 }).map((_, i) => {
      ...
      return {
        dia: new Date(f).toLocaleDateString("es-PE", { weekday: "short" }),
    ...
    const tendencia = Array.from({ length: 30 }).map((_, i) => {
      ...
      return { fecha: new Date(f).getDate().toString(), asistencia: +pct.toFixed(1) };
    });
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Empleados activos" value={datos.totalActivos} icon={Users} variant="primary" trend={{ value: "+2 este mes", positive: true }} />
        ...
        <div className="lg:col-span-2 rounded-2xl bg-card border border-border p-6 shadow-soft">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={datos.tendencia}>
              ...
              <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12, fontSize: 12 }} />
              <Line
                type="monotone" dataKey="asistencia"
                stroke={COLORES.primary} strokeWidth={2.5}
                dot={false} activeDot={{ r: 5, fill: COLORES.primary }}
                fill="url(#grad)"
              />
            </LineChart>
          </ResponsiveContainer>
              ...
              {datos.recientes.map(a => {
                ...
                return (
                  <div key={a.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-base">
                    <img src={e.foto} alt={e.nombre} className="h-9 w-9 rounded-full object-cover ring-2 ring-primary/20" />
